...
class abuseIPDB():
    ...
    async def __formating_Input(self, decodedResponse):
        ...
        for response in decodedResponse:
            try:
                ...
                self.abs_lst.append([code, err_msg])
                msg = "[-] " + "AbuseIpDB Engine Error: Formating Input Error, " + err_msg
                print(Fore.RED + Style.BRIGHT + msg + Fore.RESET + Style.RESET_ALL)
                continue

        print(Fore.CYAN + Style.BRIGHT + f"[+] Finished Processing List --> Generating " + Fore.YELLOW + f"abuseIPDB Report" + Fore.RESET)
    ...
    async def __formating_Output(self, decodedResponse, target_url):
        ...
        try:
            ...
            self.abs_lst.append([target_url, decodedResponse["data"]["abuseConfidenceScore"]])
    ...
    async def generate_Report(self, target_url, isFile=False):
        ...
        try:
            ...
            error_msg = ex.args[0]
            msg = "[-] " + "AbuseIpDB Error: " + ip + " Gererate Report Error, " + error_msg
            print(Fore.RED + Style.BRIGHT + msg + Fore.RESET + Style.RESET_ALL)
            return msg
    ...
    async def abuseipDB_Report(self, timestamp, target_url, isFile=False):
        ...
        await HTML_Report.outputHTML(config.ABUSEIPDB_REPORT_FILE_NAME, config.ABUSEIPDB_REPORT_TITLE, config.ABUSEIPDB_REPORT_SUB_TITLE, timestamp)
        return summary_lst
